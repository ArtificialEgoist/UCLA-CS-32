    ...
    // no change to the map and return false.
{
	...
	for(int i=0; i<size(); i++)
	{
		if(pairs[i].key==key)
		{
			...
			//pairs[size()-1].key=KeyType(NULL);
			//pairs[size()-1].value=ValueType(NULL);
			lastIndex--;
			return true;
		}
	}
    ...
    // Exchange the contents of this map with the other one.
{
	...
	while(!other.empty())
	{
		...
		other.get(0, k, v);
		other.erase(k); //essentially sets other's lastIndex to -1 again (empty)
	}
	...
	while(!empty())
	{
		...
		get(0, k, v);
		erase(k); //essentially sets lastIndex of current map to -1 again (empty)
	}
