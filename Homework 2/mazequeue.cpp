...
{
	queue<Coord> cQueue;
	cQueue.push(Coord(sr, sc)); //Push the starting coordinate (sr,sc) onto the coordinate stack and
	...
	while(!cQueue.empty()) //While the stack is not empty,
	{
		...
		cQueue.pop(); //Pop the top coordinate off the stack. This gives you the current (r,c) location that your algorithm is exploring.
